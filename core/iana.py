...
# ── CHROMADB ──────────────────────────────────────────────────────
def _run_pipeline():
    global nome_usuario, id_conversa, msg_final, banco_ok, colecao, modelo, contexto, bloco_contexto, chave, modelo_gemini, url_api, instrucao_humor, resposta
...
"""
    ...
    def chamar_gemini():
        ...
        if os.getenv('IANA_LOCAL_ONLY', 'false').lower() == 'true':
            sys.stderr.write('[Gemini] modo local ativo — não usando Gemini\n')
            return None
        if not chave:
...
def main():
    global nome_usuario, id_conversa, msg_final
    nome_usuario = sys.argv[1].strip() if len(sys.argv) > 1 else 'Jogador'
    id_conversa  = sys.argv[2].strip() if len(sys.argv) > 2 else 'chat_geral'
    msg_final    = ' '.join(sys.argv[3:]).strip() if len(sys.argv) > 3 else ''
