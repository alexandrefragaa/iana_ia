...
async function realizarLogin() {
    ...
    try {
        ...
        const data = await res.json();
        if (!res.ok) erro('login-erro', data.erro || 'Falha no login.');
...
async function realizarCadastro() {
    ...
    try {
        ...
        const data = await res.json();
        if (!res.ok) erro('cadastro-erro', data.erro || 'Falha no cadastro.');
        else { fecharModal(); atualizarUILogado(data.usuario); }
...
async function enviarCodigo() {
    ...
    try {
        ...
        const data = await res.json();
        if (!res.ok) { erroEl.style.color='#f87171'; erroEl.textContent = data.erro || 'Erro.'; }
        else {
...
async function mudarSenha() {
    ...
    try {
        ...
        const data = await res.json();
        if (!res.ok) erro('codigo-erro', data.erro || 'Código inválido.');
